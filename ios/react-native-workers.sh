...
echo "Starting react-native-workers inside ios directory with CONFIG: ${CONFIGURATION}"
...
DEST=$CONFIGURATION_BUILD_DIR/$UNLOCALIZED_RESOURCES_FOLDER_PATH/src/workers

echo $DEST
mkdir -p $DEST

# Get list of entry files (change src/workers/*Worker.js to match the location and filename pattern of your workers)
for ENTRY_FILE in ${1:-src/workers/*Worker.js}
do
  ...
  BUNDLE_FILE="$DEST/$ENTRY_FILE_BASENAME.jsbundle"
    ...
    echo "ENTRY_FILE_BASENAME: ${ENTRY_FILE_BASENAME}"
